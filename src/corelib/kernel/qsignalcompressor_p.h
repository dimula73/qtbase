 ...
 */
class Q_CORE_EXPORT QSignalCompressor : public QObject
{
    Q_OBJECT
...
public:
    enum Mode {
        ...
        UNDEFINED /* QSignalCompressor is created without an explicit mode */
    };
...
public:
    QSignalCompressor();
    QSignalCompressor(int delay, Mode mode, QObject *parent = 0);
    QSignalCompressor(int delay, Mode mode, SlowHandlerMode slowHandlerMode, QObject *parent = 0);
    bool isActive() const;
